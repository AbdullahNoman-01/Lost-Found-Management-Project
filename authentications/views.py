...
from lost_items.models import ReportLostItem
from found_items.models import ReportFoundItem
...
def ProfileView(request):
    lost_items = ReportLostItem.objects.filter(
        user=request.user
    ).order_by("-date_lost")

    found_items = ReportFoundItem.objects.filter(
        user=request.user
    ).order_by("-date_found")
...
def Edit_ProfileView(request):
    ...
    return render(
        request,
        "authentications/edit_profile.html"
    )
